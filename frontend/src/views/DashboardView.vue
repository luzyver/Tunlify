...
<template>
  <div class="page">
    <div class="page-header">
      <div>
        <span class="page-badge">Console &middot; Status</span>
    ...
    <div v-if="status" class="mb-8">
      <div class="stat-grid">
        <div class="card stat-card card-hover">
          ...
          <p class="stat-value tabular-nums">{{ status.uptime || '\u2014' }}</p>
        </div>
        <div class="card stat-card card-hover">
          <p class="stat-label">Ingress rules</p>
          <p class="stat-value tabular-nums">{{ status.ingress_count ?? 0 }}</p>
        </div>
        <div class="card stat-card card-hover">
          <p class="stat-label">Tunnel</p>
          <p class="stat-value font-mono text-truncate" style="font-size: 20px;">{{ status.tunnel_name || 'n/a' }}</p>
        </div>
        <div class="card stat-card card-hover">
          <p class="stat-label">Memory</p>
          <p class="stat-value tabular-nums">{{ memory.used }}</p>
          <p v-if="memory.total" class="stat-label" style="margin-top: 2px; margin-bottom: 0; color: #71717A;">of {{ memory.total }}</p>
        </div>
      </div>
    </div>
    ...
    <p v-if="!status" class="empty">Loading status...</p>
  </div>
</template>
